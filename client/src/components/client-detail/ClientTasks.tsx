...
import { Loader2, Plus, Edit2, Trash2, ListChecks, AlertTriangle, CheckCircle2, XCircle, Clock } from "lucide-react";
...
const STATUS_CONFIG = {
  Ongoing:  { color: "#2563EB", bg: "#DBEAFE", icon: Clock },
  Complete: { color: "#16A34A", bg: "#DCFCE7", icon: CheckCircle2 },
...
export default function ClientTasks({ clientId }: Props) {
  ...
  const { data: tasks, isLoading } = trpc.admin.getClientTasks.useQuery({ clientProfileId: clientId });
  ...
  const invalidate = () => utils.admin.getClientTasks.invalidate({ clientProfileId: clientId });

  const createMutation = trpc.admin.createClientTask.useMutation({
    onSuccess: () => { toast.success("Task added"); invalidate(); setShowDialog(false); },
    onError: (e) => toast.error(e.message),
  });
  const updateMutation = trpc.admin.updateClientTask.useMutation({
    onSuccess: () => { toast.success("Task updated"); invalidate(); setShowDialog(false); },
    onError: (e) => toast.error(e.message),
  });
  const deleteMutation = trpc.admin.deleteClientTask.useMutation({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        {[
          { label: "Total Tasks", value: String(total), color: "text-slate-700", bg: "bg-slate-50", icon: ListChecks },
                  ...
                  {tasks.map((t) => {
                    ...
                    return (
                      <TableRow key={t.id}>
                        ...
                        <TableCell className="text-xs max-w-[120px] truncate">{t.resultNotes || "—"}</TableCell>
                        <TableCell>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingId ? "Edit Task" : "Add Client Task"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1">
              <Label className="text-xs">Task Title <span className="text-destructive">*</span></Label>
              <Input value={form.taskTitle} onChange={setField("taskTitle")} placeholder="e.g. Submit bank statements" className="h-8 text-sm" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <Textarea value={form.taskDetails} onChange={setField("taskDetails")} placeholder="Describe what the client needs to do..." className="text-sm min-h-[70px]" />
            </div>
            <div className="space-y-1">
